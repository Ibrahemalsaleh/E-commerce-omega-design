...
@section('title', 'Edit Product') {{-- Translated title --}}

@section('header', 'Edit Product: ' . $product->name) {{-- Translated header --}}
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        @foreach($collections as $collection)
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="collections[]" value="{{ $collection->id }}" id="collection_{{ $collection->id }}"
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        @if($product->image_url)
                            <div class="mb-2">
                                {{-- Assuming asset() handles the correct path --}}
                                <img src="{{ asset($product->image_url) }}" alt="{{ $product->name }}" class="img-thumbnail" width="200">
                            </div>
                        @endif
                        <input type="file" class="form-control @error('image') is-invalid @enderror" id="image" name="image">
                        <small class="form-text text-muted">Leave this field empty if you don't want to change the image.</small> {{-- Translated hint text --}}
                        @error('image')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="card mt-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="is_new_arrival" id="is_new_arrival" {{ old('is_new_arrival', $product->is_new_arrival) ? 'checked' : '' }}>
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_bestseller" id="is_bestseller" {{ old('is_bestseller', $product->is_bestseller) ? 'checked' : '' }}>
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save me-2"></i> Save Changes {{-- Translated button text and adjusted icon margin for LTR --}}
                </button>
            </div>
        </form>
    </div>
